...
public class BulkLoadUtils {
    ...
    public static List<Tuple2<Long, byte[][]>> getCutPoints(double sampleFraction,
        ...
        for (Long conglomId : numPartitions.keySet()) {
            ...
            List<byte[]> cutPointList = Lists.newArrayList();
            ...
            for (int i = 1; i < numPartition; ++i) {
                ...
                if(prev == null || org.apache.hadoop.hbase.util.Bytes.compareTo(prev, bytes) != 0) {
                    cutPointList.add(bytes);
                    prev = bytes;
                }
            }
            byte[][] cutPoints = new byte[cutPointList.size()][];
            cutPoints = cutPointList.toArray(cutPoints);
